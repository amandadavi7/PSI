...
namespace cuckoofilter {
    ...
    class CuckooFilter {
        ...
        inline uint32_t TagHash(uint32_t hv) const {
            uint32_t tag;
            tag = hv & (((uint32_t)1 << bits_per_item) - 1);  
            tag += (tag == 0);	    
        ...
        inline void GenerateIndexTagHashPSI(uint64_t hash,
                                         size_t* index,
                                         uint32_t* tag) const {
            //const uint64_t hash = HashUtil::SuperFastHash(item);
            *index = IndexHash(hash >> 32); // high
            *tag = TagHash(hash); // low
...
//		std::cout << "num_buckets: " << num_buckets << std::endl;
        ...
        Status AddPSI(uint64_t hash);

        // Report if the item is inserted, with false positive rate.
        Status Contain(const ItemType& item) const;
	Status ContainPSI(uint64_t item) const;
        ...
        // write in a file
        void WriteFile(FILE* arq);
	
	// read from a file
        void ReadFile(FILE* arq);
	
	// write in a memory
	void WriteMem(uint8_t** buffer);
	
	// read from a memory
	void ReadMem(uint8_t** buffer);
    ...
    void CuckooFilter<ItemType, bits_per_item, TableType>::WriteFile(FILE* arq){
    
	  table_->WriteFile(arq); 
    ...
    void CuckooFilter<ItemType, bits_per_item, TableType>::ReadFile(FILE* arq){
    ...
    void CuckooFilter<ItemType, bits_per_item, TableType>::WriteMem(uint8_t** buffer){
    
	  table_->WriteMem(buffer); 
    ...
    void CuckooFilter<ItemType, bits_per_item, TableType>::ReadMem(uint8_t** buffer){
    
	  table_->ReadMem(buffer); 
    ...
    template <typename ItemType, size_t bits_per_item,
              template<size_t> class TableType>
    Status
    CuckooFilter<ItemType, bits_per_item, TableType>::AddPSI(
            uint64_t hash) {
        size_t i;
        uint32_t tag;
	...
	GenerateIndexTagHashPSI(hash, &i, &tag);
        return AddImpl(i, tag);
    }
    ...
    CuckooFilter<ItemType, bits_per_item, TableType>::AddImpl(
        ...
        for (uint32_t count = 0; count < kMaxCuckooCount; count++) {
            ...
            if (table_->InsertTagToBucket(curindex, curtag, kickout, oldtag)) {
                num_items_++;
//		std::cout << "IndexAddImp:" << curindex << std::endl;
//		std::cout << "TagAddImp:" << curtag << std::endl;	
                return Ok;
            }
            if (kickout) {
                curtag = oldtag;
//		std::cout << "kickout" << std::endl;
	...
	assert(i1 == AltIndex(i2, tag));
     ...
     template <typename ItemType,
              ...
              template<size_t> class TableType>   
    Status
    CuckooFilter<ItemType, bits_per_item, TableType>::ContainPSI(
            uint64_t key) const {
        bool found = false;
        size_t i1, i2;
        uint32_t tag;

        GenerateIndexTagHashPSI(key, &i1, &tag);
        i2 = AltIndex(i1, tag);
