...
bool qf_initfile(QF *qf, uint64_t nslots, uint64_t key_bits, uint64_t value_bits, enum qf_hashmode hash, uint32_t seed, char* filename)
{
	...
	qf->metadata = (qfmetadata *)mmap(NULL, total_num_bytes, PROT_READ | PROT_WRITE, MAP_SHARED, qf->runtimedata->f_info.fd, 0);
	...
	uint64_t init_size = qf_init(qf, nslots, key_bits, value_bits, hash, seed, qf->metadata, total_num_bytes);
...
#endif
	qf->metadata = (qfmetadata *)mmap(NULL, sb.st_size, PROT_READ | PROT_WRITE, MAP_SHARED, qf->runtimedata->f_info.fd, 0);
	if (qf->metadata == MAP_FAILED) {
		perror("Couldn't mmap metadata.");
		exit(EXIT_FAILURE);
	}
