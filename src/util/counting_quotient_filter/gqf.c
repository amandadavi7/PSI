...
#define MAX_VALUE(nbits) (((__uint128_t)1 << (nbits)) - 1)
...
#ifdef LOG_WAIT_TIME
static inline bool qf_spin_lock(QF *qf, volatile int *lock, uint64_t idx, uint8_t flag)
{
	...
	if (GET_WAIT_FOR_LOCK(flag) != QF_WAIT_FOR_LOCK) {
		...
		qf->runtimedata->wait_times[idx].total_time_single += BILLION * (end.tv_sec -  start.tv_sec) +
		...
		if (!__sync_lock_test_and_set(lock, 1)) {
			...
			while (__sync_lock_test_and_set(lock, 1))
				while (*lock);
			clock_gettime(CLOCK_THREAD_CPUTIME_ID, &end);
			qf->runtimedata->wait_times[idx].total_time_spinning += BILLION * (end.tv_sec - start.tv_sec) +
				end.tv_nsec - start.tv_nsec;
		}
		ret = true;
	}
...
static bool qf_lock(QF *qf, uint64_t hash_bucket_index, bool small, uint8_t runtime_lock)
{
	uint64_t hash_bucket_lock_offset  = hash_bucket_index % NUM_SLOTS_TO_LOCK;
	if (small) {
#ifdef LOG_WAIT_TIME
		if (!qf_spin_lock(qf, &qf->runtimedata->locks[hash_bucket_index/NUM_SLOTS_TO_LOCK],  hash_bucket_index/NUM_SLOTS_TO_LOCK, runtime_lock))
			return false;
		if (NUM_SLOTS_TO_LOCK - hash_bucket_lock_offset <= CLUSTER_SIZE) {
			if (!qf_spin_lock(qf, &qf->runtimedata->locks[hash_bucket_index/NUM_SLOTS_TO_LOCK+1], hash_bucket_index/NUM_SLOTS_TO_LOCK+1, runtime_lock)) {
		...
		if (hash_bucket_index >= NUM_SLOTS_TO_LOCK && hash_bucket_lock_offset <= CLUSTER_SIZE) {
			if (!qf_spin_lock(qf, &qf->runtimedata->locks[hash_bucket_index/NUM_SLOTS_TO_LOCK-1], runtime_lock))
				return false;
		}
		if (!qf_spin_lock(qf, &qf->runtimedata->locks[hash_bucket_index/NUM_SLOTS_TO_LOCK], runtime_lock)) {
			if (hash_bucket_index >= NUM_SLOTS_TO_LOCK && hash_bucket_lock_offset <= CLUSTER_SIZE)
		...
		if (hash_bucket_index >= NUM_SLOTS_TO_LOCK && hash_bucket_lock_offset <= CLUSTER_SIZE) {
			if
				(!qf_spin_lock(&qf->runtimedata->locks[hash_bucket_index/NUM_SLOTS_TO_LOCK-1], runtime_lock))
				return false;
		}
...
static inline void shift_remainders(QF *qf, const uint64_t start_index, const uint64_t empty_index)
{
	...
	while (last_word != first_word) {
		*REMAINDER_WORD(qf, last_word) = shift_into_b(*REMAINDER_WORD(qf, last_word-1), *REMAINDER_WORD(qf, last_word),
...
void qf_dump_metadata(const QF *qf) {
	printf("Slots: %lu Xnslots: %lu Occupied: %lu Elements: %lu Distinct: %lu\n",
				 qf->metadata->nslots,
				 qf->metadata->xnslots,
...
static inline void shift_runends(QF *qf, int64_t first, uint64_t last, uint64_t distance)
{
	...
	if (last_word != first_word) {
		METADATA_WORD(qf, runends, 64*last_word) = shift_into_b(METADATA_WORD(qf, runends, 64*(last_word-1)), METADATA_WORD(qf, runends, 64*last_word),
																														0, bend, distance);
		bend = 64;
		last_word--;
		while (last_word != first_word) {
...
static inline bool insert_replace_slots_and_shift_remainders_and_runends_and_offsets(QF	*qf, int operation, uint64_t bucket_index,uint64_t overwrite_index, 
                                                                                const uint64_t *remainders, uint64_t	total_remainders,  uint64_t noverwrites)
{
	uint64_t empties[67];
	uint64_t i;
	int64_t j;
...
static inline int remove_replace_slots_and_shift_remainders_and_runends_and_offsets(QF *qf, int operation, uint64_t	bucket_index, uint64_t overwrite_index,
                                                                                const uint64_t	*remainders, uint64_t total_remainders, uint64_t old_length)
{
	uint64_t i;
		...
		if (is_occupied(qf, hash_bucket_index)) {
			...
			while (current_remainder < hash_remainder && runstart_index <=
				...
				if (runstart_index < runend_index && get_slot(qf, runstart_index + 1) < current_remainder) {
					...
					while (runstart_index < runend_index && get_slot(qf, runstart_index) != current_remainder)
						runstart_index++;
					runstart_index++;
...
static inline int insert(QF *qf, __uint128_t hash, uint64_t count, uint8_t runtime_lock)
{
	int ret_distance = 0;
	uint64_t hash_remainder           = hash & BITMASK(qf->metadata->bits_per_slot);
	uint64_t hash_bucket_index        = hash >> qf->metadata->bits_per_slot;
	...
	if (might_be_empty(qf, hash_bucket_index) && runend_index == hash_bucket_index) {
		...
		if (!is_occupied(qf, hash_bucket_index)) { /* Empty bucket, but its slot is occupied. */
			...
			if (current_remainder < hash_remainder) {
				...
				ret = insert_replace_slots_and_shift_remainders_and_runends_and_offsets(qf, is_runend(qf, current_end) ? 1 : 2, hash_bucket_index,  runstart_index, p, &new_values[67] - p, current_end - runstart_index + 1);
			if (!ret)
				return QF_NO_SPACE;
			ret_distance = runstart_index - hash_bucket_index;
				...
				uint64_t *p = encode_counter(qf, hash_remainder, count, &new_values[67]);
				ret = insert_replace_slots_and_shift_remainders_and_runends_and_offsets(qf,  2, /* Insert to bucket */ hash_bucket_index, runstart_index, 
																				p, &new_values[67] - p, 0);
				if (!ret)
					return QF_NO_SPACE;
				modify_metadata(qf, &qf->metadata->ndistinct_elts, 1);
...
inline static int _remove(QF *qf, __uint128_t hash, uint64_t count, uint8_t	runtime_lock)
{
	...
	ret_numfreedslots = remove_replace_slots_and_shift_remainders_and_runends_and_offsets(qf,only_item_in_the_run, hash_bucket_index, runstart_index,
...
uint64_t qf_init(QF *qf, uint64_t nslots, uint64_t key_bits, uint64_t value_bits, enum qf_hashmode hash, uint32_t seed, void* buffer, uint64_t buffer_len)
{
	...
	qf->metadata->total_size_in_bytes_plus_metadada = total_num_bytes;		
...
bool qf_malloc(QF *qf, uint64_t nslots, uint64_t key_bits, uint64_t value_bits, enum qf_hashmode hash, uint32_t seed)
{
...
{
	QF new_qf;
	if (!qf_malloc(&new_qf, nslots, qf->metadata->key_bits, qf->metadata->value_bits, qf->metadata->hash_mode, qf->metadata->seed))
		return false;
	...
	do {
		...
		if (ret < 0) {
            fprintf(stderr, "Failed to insert key: %lx %lx into the new CQF.\n", (uint64_t)(key >> 64), (uint64_t)key);
			return ret;
		}
		ret_numkeys++;
	} while(!qfi_end(&qfi));
...
uint64_t qf_resize(QF* qf, uint64_t nslots, void* buffer, uint64_t buffer_len)
{
	...
	uint64_t init_size = qf_init(&new_qf, nslots, qf->metadata->key_bits, qf->metadata->value_bits,qf->metadata->hash_mode, qf->metadata->seed, 
															 buffer, buffer_len);
...
int qf_insert(QF *qf, __uint128_t key, uint64_t value, uint64_t count, uint8_t flags)
{
	...
	if (GET_KEY_HASH(flags) != QF_KEY_IS_HASH) {
		if (qf->metadata->hash_mode == QF_HASH_DEFAULT)
			key = MurmurHash3_x64_128(((void *)&key), sizeof(key), qf->metadata->seed) % qf->metadata->range; 
		else if (qf->metadata->hash_mode == QF_HASH_INVERTIBLE)
			key = hash_128(key, BITMASK(qf->metadata->key_bits));
	}
	__uint128_t hash = 	(key << (__uint128_t) qf->metadata-> value_bits) | (value & BITMASK(qf->metadata->value_bits));
	int ret;
	if (count == 1)
...
int qf_set_count(QF *qf, uint64_t key, uint64_t value, uint64_t count, uint8_t  flags)
{
	if (count == 0)
		return 0;
...
int qf_remove(QF *qf, uint64_t key, uint64_t value, uint64_t count, uint8_t flags)
{
	if (count == 0)
		return true;
...
uint64_t qf_count_key_value(const QF *qf, __uint128_t key, uint64_t value, uint8_t flags)
{
	if (GET_KEY_HASH(flags) != QF_KEY_IS_HASH) {
		if (qf->metadata->hash_mode == QF_HASH_DEFAULT)
			key = MurmurHash3_x64_128(((void *)&key), sizeof(key), qf->metadata->seed) % qf->metadata->range;
...
uint64_t qf_query(const QF *qf, __uint128_t key, uint64_t *value, uint8_t flags)
{
	if (GET_KEY_HASH(flags) != QF_KEY_IS_HASH) {
		if (qf->metadata->hash_mode == QF_HASH_DEFAULT)
			key = MurmurHash3_x64_128(((void *)&key), sizeof(key), qf->metadata->seed) % qf->metadata->range;
	...
	int64_t hash_bucket_index = hash >> (__uint128_t) qf->metadata->key_remainder_bits;

	if (!is_occupied(qf, hash_bucket_index))
		return 0;

	int64_t runstart_index = hash_bucket_index == 0 ? 0 : run_end(qf, hash_bucket_index-1)
		+ 1;
	if (runstart_index < hash_bucket_index)
		runstart_index = hash_bucket_index;
...
int64_t qf_get_unique_index(const QF *qf, __uint128_t key, uint64_t value, uint8_t flags)
{
	if (GET_KEY_HASH(flags) != QF_KEY_IS_HASH) {
		if (qf->metadata->hash_mode == QF_HASH_DEFAULT)
			key = MurmurHash3_x64_128(((void *)&key), sizeof(key), qf->metadata->seed) % qf->metadata->range;
...
int64_t qf_iterator_key_value(const QF *qf, QFi *qfi, __uint128_t key, uint64_t value, uint8_t flags)
{
	...
	uint64_t hash = (key << (__uint128_t)qf->metadata->value_bits) | (value & BITMASK(qf->metadata->value_bits));
	uint64_t hash_remainder   = hash & BITMASK(qf->metadata->bits_per_slot);
	uint64_t hash_bucket_index = hash >> qf->metadata->bits_per_slot;
	bool flag = false;
...
static int qfi_get(const QFi *qfi, __uint128_t *key, uint64_t *value, uint64_t *count)
{
	if (qfi_end(qfi))
		return QFI_INVALID;
	...
	*key = (qfi->run << (__uint128_t)qfi->qf->metadata->key_remainder_bits) | current_remainder;
	*count = current_count;

	return 0;
}

int qfi_get_key(const QFi *qfi, __uint128_t *key, uint64_t *value, uint64_t *count)
{
	*key = 0;
    *value = *count = 0;
	...
	if (ret == 0) {
		if (qfi->qf->metadata->hash_mode == QF_HASH_DEFAULT) {
			...
			*key = hash_128i(*key, BITMASK(qfi->qf->metadata->key_bits));
	}

	return ret;
}

int qfi_get_hash(const QFi *qfi, __uint128_t *key, uint64_t *value, uint64_t *count)
{
	*key =0;
    *value = *count = 0;
...
void qf_merge(const QF *qfa, const QF *qfb, QF *qfc)
{
	...
	__uint128_t keya, keyb;
   	uint64_t counta, valuea, valueb, countb;
...
double load_factor(const QF *qf)
{
	if(qf->metadata->noccupied_slots==0){      
	  return 0;
	}
	else{
	  return (double)qf->metadata->noccupied_slots/(double)qf->metadata->xnslots;
	}
}
      
void write_mem(QF *qf, uint8_t** buffer_block)
{	  
	*buffer_block = (uint8_t*)malloc(qf->metadata->total_size_in_bytes + sizeof(qfmetadata));
	memcpy((*buffer_block), qf->metadata, sizeof(qfmetadata));      	 
	memcpy((*buffer_block) + sizeof(qfmetadata), qf->blocks, qf->metadata->total_size_in_bytes);      	     
}

	
void read_mem(QF *qf, uint8_t* buffer_metadata, uint8_t* buffer_block)
{ 

	/* initlialize the locks in the QF */
	qf->runtimedata = (qfruntime *)calloc(sizeof(qfruntime), 1);	
        qf->metadata = (qfmetadata*) buffer_metadata;	
	qf->runtimedata->num_locks = (qf->metadata->xnslots/NUM_SLOTS_TO_LOCK)+2;
	qf->runtimedata->metadata_lock = 0;
	/* initialize all the locks to 0 */
	qf->runtimedata->locks = (volatile int *)calloc(qf->runtimedata->num_locks, sizeof(volatile int));
	qf->blocks = (qfblock*) buffer_block;
